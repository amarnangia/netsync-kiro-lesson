...
The Stock Data Explorer is a Streamlit-based web application that provides an interactive interface for visualizing stock price data and fetching real historical data from external APIs. The application uses a staged development approach:

- **Stage 1**: Mock data generation and visualization (completed)
- **Stage 2**: API integration for fetching and displaying raw data (current)
- **Stage 3**: Data processing and integration (future)

The design focuses on clean separation between UI components, data generation, API fetching, and visualization logic, with modularity to support incremental feature development.
...
│  │  - Title                             │  │
│  │  - Stock Selector                    │  │
│  │  - Plot Containers (Stage 1)         │  │
│  │  - Raw API Data Display (Stage 2)    │  │
│  └──────────────────────────────────────┘  │
│              │                              │
│              ▼                              │
│  ┌──────────────────────────────────────┐  │
...
│  ┌──────────────────────────────────────┐  │
│  │   Visualization Layer                │  │
│  │  - Dot Plot Renderer (Stage 1)       │  │
│  │  - JSON Display (Stage 2)            │  │
│  └──────────────────────────────────────┘  │
└─────────────────────────────────────────────┘
```

The application uses Streamlit's reactive model where user interactions trigger automatic re-execution of the script, updating the UI accordingly. Stage 2 adds API fetching capabilities while maintaining Stage 1 mock data visualizations independently.
...
### 5. API Configuration (Stage 2)

API key management using environment variables or placeholder:

```python
import os

# API Configuration
# IMPORTANT: Insert your API key here or set MASSIVE_API_KEY environment variable
API_KEY = os.getenv("MASSIVE_API_KEY", "YOUR_API_KEY_HERE")
API_BASE_URL = "https://api.massive.com/v1"  # Example URL
```

**Security considerations:**
- Never commit API keys to version control
- Use environment variables for production deployments
- Provide clear placeholder for manual key insertion
- Document where users should insert their API key

### 6. API Fetcher Component (Stage 2)

Function signature:
```python
def fetch_stock_data(ticker: str, api_key: str) -> dict | None
```

**Parameters:**
- `ticker`: Stock ticker symbol to fetch data for
- `api_key`: API authentication key

**Returns:**
- Dictionary containing raw API response data
- `None` if request fails or returns empty data

**Implementation approach:**
- Use `requests` library for HTTP calls
- Request up to 2 years of historical data
- Implement rate limiting (5 calls per minute)
- Handle connection errors, timeouts, and invalid responses
- Return raw JSON response without processing
- Log errors for debugging

**Rate Limiting Strategy:**
- Track timestamp of last API call
- Enforce minimum 12-second delay between calls (5 calls/minute = 12 seconds/call)
- Display waiting message to user if rate limit would be exceeded
- Use Streamlit session state to persist rate limit tracking across reruns

### 7. Raw Data Display Component (Stage 2)

Function signature:
```python
def display_raw_api_data(api_response: dict) -> None
```
...
- Add section header "Raw API Data (Stage 2)"
- Display in expandable section for better UX
- Show data structure clearly with proper indentation

### 8. Error Handling Component (Stage 2)
...
**Error types:**
- Connection failure: "Unable to connect to API"
- Empty response: "No data available for this stock"
- Rate limit exceeded: "Please wait before making another request"
- Invalid API key: "API authentication failed"
- Timeout: "Request timed out, please try again"

## Data Models

### StockData (Conceptual - Stage 1)
...
Raw API response structure (example based on typical stock APIs):
...
**Note:** Actual structure depends on Massive API specification. Stage 2 displays raw response without processing.
